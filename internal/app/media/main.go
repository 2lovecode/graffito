...
func NewFileTypeCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "file-type",
		Short: "文件类型识别工具",
		Long: `通过文件头（Magic Number）识别文件类型。

支持识别多种文件格式：
  - 图片：jpg, png, gif, bmp, tif, psd
  - 文档：pdf, doc, docx, xls, ppt
  - 音频：mp3, wav, mid
  - 视频：mp4, avi, wmv, flv
  - 压缩：zip, rar, gz, jar
  - 其他：html, css, js, xml, json 等

示例:
  graffito cli tools file-type --file image.jpg
`,
		Run: func(cmd *cobra.Command, args []string) {
			if file == "" {
				fmt.Fprintf(cmd.ErrOrStderr(), "错误: 请指定文件路径\n")
			...
			f, err := os.Open(file)
			if err != nil {
				logging.Errorf("打开文件失败: %v", err)
				fmt.Fprintf(cmd.ErrOrStderr(), "错误: 打开文件失败: %v\n", err)
				return
			}
			defer f.Close()
			...
			if err != nil {
				logging.Errorf("识别文件类型失败: %v", err)
				fmt.Fprintf(cmd.ErrOrStderr(), "错误: 识别文件类型失败: %v\n", err)
				return
			}
	...
	cmd.Flags().StringVarP(&file, "file", "f", "", "文件路径")
	return cmd
}
